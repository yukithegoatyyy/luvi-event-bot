...
const DISCORD_TOKEN = process.env.DISCORD_TOKEN ?? process.env.APIKEY;
...
for (const [name, value] of Object.entries({ CHANNEL_ID, LUVI_BOT_ID })) {
  if (!value) {
...
if (!DISCORD_TOKEN) {
  throw new Error('Missing required environment variable: DISCORD_TOKEN or APIKEY');
}
...
function startHealthServer() {
  ...
  const server = createServer((request, response) => {
    if (request.url === '/health') {
      ...
      response.end(JSON.stringify({ status: 'ok', loggedIn: client.isReady() }));
      return;
    }

    response.writeHead(200, { 'Content-Type': 'text/plain' });
    response.end('Luvi Event Bot is running.');
  });
